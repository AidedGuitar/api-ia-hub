...
def read_applications(skip: int = 0, limit: int = 20, db: Session = Depends(get_db)):
    return get_apps(db, skip, limit)

@router.get("/{app_id}", response_model=ApplicationRead)
def read_application(app_id: UUID, db: Session = Depends(get_db)):
...
def create_application(app_in: ApplicationCreate, db: Session = Depends(get_db)):
    try:
        ...
        if "application_app_name_key" in str(e.orig):
            raise HTTPException(
                status_code=status.HTTP_409_CONFLICT,
                detail="El nombre de la aplicación ya existe."
            )
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="Error al crear la aplicación."
        )
